...
struct draw_command
{
  static constexpr dispatch_function DISPATCH_FUNCTION =
      &graphic_backend::dispatch_draw;
...
{
  static constexpr dispatch_function DISPATCH_FUNCTION =
      &graphic_backend::dispatch_set_uniform;

  shader_program_handle shader;
  uniform_type          type;
  // FIXME: Make uniform handle to avoid having pointers that may go out of
  // scope
  const char *          name;
  TValue                data;
};
