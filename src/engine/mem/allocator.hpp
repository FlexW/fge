...
template <typename TSize> struct alloc_result
{
  void *data;
  TSize allocated_size;
};
...
template <typename TSize>
alloc_result<TSize> allocate_from_buffer(uintptr_t  buffer,
                                        TSize       allocated_size,
                                        std::size_t size,
                                        std::size_t alignment)
{
  ...
  alloc_result<TSize> result{};
  result.data           = reinterpret_cast<void *>(alloc_start);
  result.allocated_size = new_allocated_size;
